...
struct KeyValuePairTests {
    ...
    @Test func encodeVarint() {
        let pair: KeyValuePair = .init(type: 0x02, value: .varint(10))
        #expect(pair.encode() == Data([0x02, 0x0A]))
    }

    @Test func encodeBytes() {
        let pair: KeyValuePair = .init(type: 0x01, value: .bytes(Data("hi".utf8)))
        #expect(pair.encode() == Data([0x01, 0x02, 0x68, 0x69]))
    }

    @Test func roundTripVarint() throws {
        let pair: KeyValuePair = .init(type: 0x02, value: .varint(1024))
        let decoded: KeyValuePair = try .decode(from: .init(data: pair.encode()))
    ...
    @Test func roundTripBytes() throws {
        let pair: KeyValuePair = .init(type: 0x05, value: .bytes(Data("example.com".utf8)))
        let decoded: KeyValuePair = try .decode(from: .init(data: pair.encode()))
